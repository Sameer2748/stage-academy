...
export default function RecentSessions({ sessions }: RecentSessionsProps) {
  function getScoreColor(score?: number) {
    ...
    if (score >= 8) return "bg-green-50 text-green-600 border-green-200";
    if (score >= 6) return "bg-orange-50 text-orange-600 border-orange-200";
  ...
  return (
    <Card className="col-span-3 bg-white border-slate-200 flex flex-col h-full">
      <CardHeader>
        <CardTitle className="text-lg font-semibold flex items-center gap-2">
          <Mic2 className="h-5 w-5 text-indigo-600" /> Recent Sessions
        </CardTitle>
              ...
              {sessions.map((session) => (
                ...
                >
                  <div className="space-y-1">
                    <p className="font-medium text-sm leading-none group-hover:text-indigo-600 transition-colors">
