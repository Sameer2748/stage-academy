  ...
  const renderText = (text: string) => {
    ...
    return parts.map((part, i) => {
      if (fillerWordKeys.includes(part.toLowerCase())) {
        return <span key={i} className="bg-orange-500/20 text-orange-600 px-1 rounded-sm mx-0.5">{part}</span>;
      }
      return part;
    });
  };
  ...
  return (
    <Card className="bg-white border-slate-200 flex flex-col h-[500px]">
        ...
        <div className="flex items-center gap-4 text-sm">
          <label className="flex items-center gap-2 cursor-pointer text-slate-500 hover:text-slate-900 transition-colors">
            <input 
               ...
               className="accent-indigo-500 bg-slate-100 border-slate-300 rounded"
            />
      ...
      <CardContent className="flex-1 overflow-hidden p-0 relative">
              ...
              paragraphs.map((p, pIdx) => (
                <div key={pIdx} className="group">
                  <div className="flex gap-4">
                    <span className="text-xs text-slate-500 font-mono w-12 pt-1 opacity-50 group-hover:opacity-100 transition-opacity">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-8 bg-gradient-to-t from-slate-50 to-transparent z-10 pointer-events-none" />
      </CardContent>
    </Card>
  );
}
